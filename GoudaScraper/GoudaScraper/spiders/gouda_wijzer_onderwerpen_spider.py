...
class OnderwerpenSpider(scrapy.Spider):
    ...
    def parse_onderwerpen(self, response):
        onderwerpen = response.css(
            "a.articleMenuPanel__subjectAnchor::attr(href)").getall()
        onderwerp_titles = response.css(
            "a.articleMenuPanel__subjectAnchor span::text").getall()

        assert len(onderwerpen) == len(
            onderwerp_titles), "Mismatch in number of onderwerpen and titles"
        ...
        for i, onderwerp in enumerate(onderwerpen):
            ...
            yield scrapy.Request(next_page, callback=self.parse_onderwerp_artikels,
    ...
    def parse_onderwerp_artikels(self, response):
        ...
        for artikel in onderwerp_artikels:
            ...
            artikelen = artikel.css(
                "ul.indexSubjectSubItemContainer *::attr(href)").getall()
            ...
            for a in artikelen:
                ...
                yield scrapy.Request(next_page, callback=self.parse_artikel,
                                     meta={"onderwerp": response.meta["onderwerp"], "subonderwerp": subonderwerp})
