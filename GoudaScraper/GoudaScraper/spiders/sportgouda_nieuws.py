...
class NieuwsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        self.logger.info(f'📦 Aantal nieuwsitems gevonden: {len(nieuwsblokken)}')
        ...
        for blok in nieuwsblokken:
            try:
                ...
                if not all([title, date, description, href]):
                    self.logger.warning(f'⚠️ Incomplete item gevonden (missende velden): {title}, {date}, {description}, {href}')
                ...
                self.logger.error(f'❌ Fout bij het verwerken van item: {e}', exc_info=True)

        next_page = response.css('ul.pagination a.page-link::attr(href)').re_first(r'page=(\d+)')
        if next_page:
