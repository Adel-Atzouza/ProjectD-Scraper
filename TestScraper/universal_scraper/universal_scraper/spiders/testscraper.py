...
BUTTON_TEXTS = [
    "load more", "see more", "show more", "next", "volgende",
    "meer laden", "meer weergeven", "next page", "→", ">>", "..."
]
    ...
    async def parse(self, response):
        ...
        for cookie_text in COOKIE_KEYWORDS:
            ...
            while True:
                ...
                await page.wait_for_timeout(1000)  # Hier nog optimaliseren met conditional wait?
        ...
        raw_text = ' '.join(response.xpath('//body//text()[normalize-space()]').getall())
        ...
        with open("output.csv", "a", newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=['url', 'raw_text'])
            irrelevant_patterns = [
                "Cookieverklaring", "[#IABV2", "var CAPTCHA", "Powered by", "Toestemming Details", "Privacyverklaring"
            ]
            for pattern in irrelevant_patterns:
                raw_text = raw_text.replace(pattern, "")
            raw_text = re.sub(r"\[\#.*?\#\]", "", raw_text)
            raw_text = re.sub(r"var\s+[A-Z_]+\s*=\s*['\"].*?['\"];", "", raw_text)
            raw_text = re.sub(r"Cookieverklaring.*?Dataduiker", "", raw_text, flags=re.DOTALL)
            writer.writerow({
                "url": response.url,
                "raw_text": raw_text
            })
