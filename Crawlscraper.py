...
def clean_text(markdown: str) -> str:
    markdown = re.sub(r"(?m)^.*\|.*\|.*$", "", markdown) # Verwijder regels die tabellen bevatten (regels met minimaal twee pipes)
    markdown = re.sub(r"\[(.*?)\]\([^)]+\)", r"\1", markdown) # Verwijder markdown-links, alleen de zichtbare tekst blijft over
    markdown = re.sub(r"#+ ", "", markdown)# Verwijder markdown headers (regels die beginnen met #)
    markdown = re.sub(r"\n{3,}", "\n\n", markdown.strip())# Vervang drie of meer nieuwe regels door maximaal twee nieuwe regels
    sentences = re.split(r'(?<=[.!?]) +', markdown) # Splits de tekst op in losse zinnen
    return " ".join(sentences[:5]).strip() # Geef maximaal de eerste vijf zinnen terug, samengevoegd tot één string
...
async def collect_internal_urls(crawler, start_url: str, batch_size=15) -> Set[str]:
    ...
    while to_visit:
        current_batch = list(to_visit)[:batch_size]# Pak een batch van URLs om tegelijk te bezoeken
        to_visit.difference_update(current_batch)# Haal deze batch uit de te bezoeken set
        visited.update(current_batch)# Voeg deze batch toe aan de bezochte set

        print(f"\n [Batch discovery] {len(discovered) + 1} URLs")
        tasks = [crawler.arun(url, crawl_config, session_id=session_id) for url in current_batch]# Start crawl-taken voor alle URLs in de batch
        results = await asyncio.gather(*tasks, return_exceptions=True)# Wacht tot alle taken klaar zijn (ook als er fouten zijn)

        # Verwerk de resultaten van de batch
        for url, res in zip(current_batch, results):
            if isinstance(res, Exception): # Log en sla fout over als er een exception was
                print(f" ❌ {url}: {res}")
                log_error(url, res)  # gebruikt default output/logs
                continue
            
            if res.success and res.html: # Alleen verder als crawl succesvol was en HTML is opgehaald
                soup = BeautifulSoup(res.html, "html.parser")
                for tag in soup.find_all("a", href=True): # Zoek alle <a href="..."> links op de pagina
                    href = tag["href"]
                    full = urljoin(url, href) # Maak van relatieve links absolute URLs
                    parsed = urlparse(full)
                    if parsed.netloc == urlparse(start_url).netloc:# Alleen interne links (zelfde domein) toevoegen
                        # Normaliseer de URL (zonder query/fragment)
                        norm = parsed.scheme + "://" + parsed.netloc + parsed.path
                        # Voeg toe als nog niet bezocht, niet in to_visit en niet uitgesloten
...
async def crawl_parallel(urls: List[str], max_concurrent: int, log_dir: str = "output/logs"):
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        for i in range(0, len(urls), max_concurrent): # Verwerk de URLs in batches van max_concurrent
            batch = urls[i:i + max_concurrent]
            tasks = [crawler.arun(url, crawl_config, session_id=f"batch_{i + j}") for j, url in enumerate(batch)] # Maak taken aan voor alle URLs in de batch
            results_batch = await asyncio.gather(*tasks, return_exceptions=True) # Wacht tot alle taken klaar zijn (ook als er fouten zijn)
