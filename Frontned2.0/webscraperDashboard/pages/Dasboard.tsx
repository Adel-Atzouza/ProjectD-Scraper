...
interface Website { id: number; url: string; }
interface Stats { total: number; active: number; completed: number; success_rate: string; }
type ProgressStatus = {
  jobId: string; progress: number; status: string;
...
export default function Dashboard() {
  ...
  const loadData = async () => {
    const [wRes, sRes] = await Promise.all([
      fetch(`${API}/websites`), fetch(`${API}/stats`)
    ]);
  ...
  const handleStart = async () => {
    ...
    jobs.forEach((j: any) => {
      newMap[j.url] = { jobId: j.job_id, progress: 0, status: "discovering", total: 0, done: 0, success: 0, failed: 0 };
    });
      ...
      for (const url of selected) {
        ...
        try {
          ...
          if (r.ok) {
            ...
            if (data.status !== "idle" && data.status !== "done" && data.status !== "stopped") {
              allStopped = false;
            }
          }
        } catch {}
  ...
  return (
    <>
      ...
      />
      <AddWebsiteModal isOpen={addModalOpen} onClose={()=>setAddModalOpen(false)} onAdd={loadData}/>
    </>
  );
}
